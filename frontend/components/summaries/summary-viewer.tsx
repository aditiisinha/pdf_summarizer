...
const SectionTitle = ({ title }: { title: string }) => {
    return (
     ...
     bg-background/80 backdrop-blur-xs z-10">
        ...
        flex items-center justify-center gap-2">{title}
            </h2>
        </div>);
}
export function SummaryViewer({ summary }: { summary: string }) {
    ...
    return (
        <Card
            className="relative px-2 h-[350px] sm:h-[450px] lg:h-[550px]
        w-full xl:w-[600px]
        overflow-hidden
        ...
        >
            ...
            <NavigationControls
                ...
                onNext={() => setCurrentSection(prev => Math.min(prev + 1, sections.length - 1))}
